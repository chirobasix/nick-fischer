...
The canonical host is a **placeholder** (`https://nickfischer.com`). Set the
final domain in `astro.config.mjs`, `src/lib/seo.ts`, the workflow's
`PUBLIC_SITE_URL`, and `public/robots.txt` (or just set `PUBLIC_SITE_URL`).
